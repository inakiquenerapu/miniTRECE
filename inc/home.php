...
<p>Files <code>.htaccess</code>, <code>index.php</code> and <code>core/conf.php</code> (<code>conf.php</code> inside a folder named <code>core</code>) are the important ones. The remaining files are just content or for customization. Let me suggest you to start a fresh installation of miniTRECE using only these three files and then begin building, adding the other files from the .zip, one by one.</p>

<p><strong>Important:</strong> in order to use Markdown as markup language, constant <code>MARKDOWN</code> must be set to <code>"true"</code> in <code>index.php</code> line 8, and the Parsedown library (<a href="https://github.com/erusev/parsedown/blob/master/Parsedown.php">Parsedown.php</a>, <a href="https://github.com/erusev/parsedown-extra/blob/master/ParsedownExtra.php">ParsedownExtra.php</a> and <a href="https://github.com/tovic/parsedown-extra-plugin/blob/master/ParsedownExtraPlugin.php">ParsedownExtraPlugin.php</a>) installed in the <code>lib/</code> folder. All those files are included in the .zip which can be downloaded GitHub.</p>
...
<p><strong>Importante:</strong> para poder usar Markdown como lenguaje de marcado, la constante <code>MARKDOWN</code> debe ser puesta como <code>"true"</code> en <code>index.php</code> línea 8, y la biblioteca Parsedown (<a href="https://github.com/erusev/parsedown/blob/master/Parsedown.php">Parsedown.php</a>, <a href="https://github.com/erusev/parsedown-extra/blob/master/ParsedownExtra.php">ParsedownExtra.php</a> y <a href="https://github.com/tovic/parsedown-extra-plugin/blob/master/ParsedownExtraPlugin.php">ParsedownExtraPlugin.php</a>) instalada en la carpeta <code>lib/</code>. Todos estos archivos están incluídos en el .zip que puedes descargar de GitHub.</p>
